...
+ (UserManager *)sharedManager {
    static UserManager *userManagers = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        userManagers = [[self alloc] init];
    });
    return userManagers;
}
...
-(void)login:(UserLoginType )loginType params:(NSDictionary *)params completion:(loginBlock)completion{
    ...
    if (loginType != kUserLoginTypePwd) {
        [MBProgressHUD showAutoMessage:@"授权中..."];
...
-(void)loginToServer:(NSDictionary *)params completion:(loginBlock)completion{
    [MBProgressHUD showAutoMessage:@"登录中..."];
...
//    [NetworkHelper POST:NSStringFormat(@"%@%@",URL_main,URL_user_auto_login) parameters:nil success:^(id responseObject) {
//        [self LoginSuccess:responseObject completion:completion];
//
//    } failure:^(NSError *error) {
//        if (completion) {
